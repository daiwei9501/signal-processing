function [label,model,W,OutU,OutV,llh] = MLGMDN(InW,InX,r,param)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
% Author: Deyu Meng(dymeng@mail.xjtu.edu.cn), and checked by Hongwei Yong
% Date: 10-31-2014
...
for j = 1:k
    %% 20190819LC
    W(IND) = W(IND) + R(:,j)/(Sigma(j));
    C(IND) = C(IND) + R(:,j)*mu(j)/(Sigma(j));
end
C(IND) = C(IND)./W(IND);

if param.method == 1
    [TempU,TempV] = amb_mfwmd(TempU,TempV,InX,sqrt(W));
end
...
if param.method == 5
    [TempU,TempV] = wals(InX-C,sqrt(W),TempU,TempV,NumIter,0.0001);  
end



function y = loggausspdf(X, mu, Sigma)
%% only for complex gauss pdf in one dimension 20190819LC
d = size(X,1);
